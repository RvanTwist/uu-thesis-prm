...
//		}
		...
		M4_Constants.StopAtOptimum = false;
		...
		Random r2 = new Random(9001);
		instance.reduceByChance(0.6, r2.nextLong());
		...
		// Not so random he
		Random r = new Random(-7982483725678938415l);
//		Random r = null;
...
//		M4_MatchingAlgorithm matching = new M4_FreeSpotMatching();
		M4_MatchingAlgorithm matching = new M4_CompleteFlowMatchingNoDelay(planning,3);
...
//		SimulatedAnnealing annealer = new SimulatedAnnealing(planning, mfms,60, 0.95, 2000, 100000);
		...
		annealer.addLocalSearchListener(new LocalSearchListenerAdaptor(){
			public void onAccept(SimulatedAnnealing anealer)
			{
				...
				if(anealer.solution instanceof M4_Planning)
				{
					((M4_Planning)anealer.solution).testConsistancy();
				}
			}
		});
		
		annealer.setTimeLimit(M4_Constants.timeLimit*100);
