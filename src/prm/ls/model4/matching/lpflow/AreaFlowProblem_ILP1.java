...
public class AreaFlowProblem_ILP1 implements AreaFlowProblem_IF
{
	double M = 24*60*60;
	...
	public AreaFlowProblem_ILP1(M4_Area a)
	{
		this.area = a;
...
//								System.out.println("Declined edge: "+e21.from+" => "+e21.to+" slack:"+slack21);
	...
	public void initialiseEdges()
	{
			...
			{	
				...
				for(M4_Worker w : seg.feasibleTransports)
				{
					...
					{ // There is only one option
						v = this.model.makeIntVar(w,1,1,0);
					}
					else
					{
						v = this.model.makeIntVar(w,0,1,0);
					}
		...
		for( M4_Worker w : this.area.workers)
		{
			...
			if(this.shifts)
			{	
					Variable v = this.model.makeIntVar(w,1,1,0);
					seg.vars.put(w,v);
				
					Variable v2 = this.model.makeIntVar(w,1,1,0);
					seg2.vars.put(w,v2);
			}
...
//*							System.out.println("Create shifts for "+seg1+" "+seg2);
							java.util.Iterator<Entry<M4_Worker, VariableInterface>> i1 = seg1.vars.entrySet().iterator();
							java.util.Iterator<Entry<M4_Worker, VariableInterface>> i2 = seg2.vars.entrySet().iterator();
							
							Entry<M4_Worker, VariableInterface> e1 = (i1.hasNext() ? i1.next() : null);
							Entry<M4_Worker, VariableInterface> e2 = (i2.hasNext() ? i2.next() : null);
